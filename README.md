...
## Lab Descriptions:
Before running each lab, ensure the virtual environment is active (instructions below) and that you are in the appropriate directory for the lab.
...
- **Lab05**- Small extraction app which reads plain text course listings from a file, and uses a model to request structured output in the form of a properly formatted JSON object. The program also validates the result using Pydantic, and writes the output to a CSV file. Finally, the formatted text is compared against a gold standard and scored for accuracy.
  - TO USE: Run score.py to view the evaluation scores from within the lab05mini directory. (The text hs already been extracted by extract.py, and the validated JSON has been added to the sections_test.csv file)
...
2) Import the necessary packages: ```pip install ollama numpy chromadb scikit-learn``` (NOTE: Lab06 also requires mysql to be installed: 
```python -m pip install --upgrade mysql-connector-python```)
3) Install and run the Ollama daemon by following the steps on [Ollama's Website](https://ollama.com/) or pasting the following command into your terminal:
   - ```curl -fsSL https://ollama.com/install.sh | sh``` (MacOS/Linux)
   - ```irm https://ollama.com/install.ps1 | iex``` (Windows)
5) Pull models used in the project by searching for them in [Ollama's Library](https://ollama.com/search) and copying the CLI command into the terminal. Ex: ```ollama pull nomic-embed-text-v2-moe```. Below is the list of models used for each lab, a * indicates it must be downloaded for the lab to run properly:
   - Lab01:
